...
import { ShoppingBag, X, Trash2, ArrowRight, ShieldCheck, Check } from 'lucide-react';
...
export const CartModal: React.FC<CartModalProps> = ({
  ...
  const totalAmount = cartItems.reduce((acc, item) => acc + item.priceFcfa, 0);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#2F241A]/70 font-light">
                {cartItems.length} {cartItems.length > 1 ? 'œuvres sélectionnées' : 'œuvre sélectionnée'}
              </p>
            </div>
          </div>
          ...
          {cartItems.length === 0 ? (
            <div className="py-12 text-center space-y-4">
              ...
              <button
                ...
                className="bg-[#A67123] hover:bg-[#8F5F1B] text-white text-xs font-medium px-5 py-2.5 rounded-xl cursor-pointer shadow-xs"
              >
          ...
          <div className="p-5 sm:p-6 bg-white border-t border-[#E8DFCE] space-y-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-[#FAF7F0] border border-[#E8DFCE] hover:bg-[#F2EADA] text-[#241710] font-medium text-xs py-3 rounded-xl transition-colors cursor-pointer"
              >
